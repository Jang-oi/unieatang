        ...
        <>
            <Table sx={{mt: '30px', textAlign: 'center', fontSize: '15px', width: '73vw'}} borderAxis="both" size="sm"
                ...
                {quizData && quizData.map((quizItem: InterviewQuizType) => (
                    <tr key={quizItem._id}>
                        <td>{quizItem.type}</td>
                        <td style={{color: '#0079F4', cursor: 'pointer'}}
                            onClick={() => setOpen(true)}>{quizItem.question}</td>
            ...
            <Modal open={open} onClose={() => setOpen(false)}>
                <ModalDialog>
                    <DialogTitle>문제 수정 및 삭제</DialogTitle>
                    <Stack spacing={2}>
                        <QuizInput/>
                    </Stack>
                </ModalDialog>
...
export const QuizInput = () => {
    ...
    return (
        <>
            ...
            <Textarea minRows={4} sx={{width: '40vw'}} placeholder={"지문을 입력 해주세요."} value={passage}
                      onChange={onPassageHandler}/>
            <Input sx={{width: '40vw'}} placeholder={"점수를 입력 해주세요."} type={"number"} value={point}
                   onChange={onPointHandler}/>
            <Textarea minRows={4} sx={{width: '40vw'}}
                      placeholder={"보기를 입력 해주세요.\n보기는 엔터로 구분 지어 입력합니다. \nex)\n 16비트 \n 12비트\n 8비트 ..."} value={choice}
                      onChange={onChoiceHandler}/>
            <Input sx={{width: '40vw'}} placeholder={"정답을 입력 해주세요."} type={"number"} value={answer}
